...
9. Install PIP3 packages into your Python virtual environment:
   ...
   ```bash
   pip3 install --upgrade setuptools json-rpc py-solc-x web3 colorlog twisted wheel toml pyzmq
   ```
...
- If the Demo test code breaks due to some error, please perform the following
  ...
  4. If some error still occurs then run : `$TCF_HOME/scripts/tcs_startup.sh -f
