...
# -------------------------------------------------------------------------
    ...
    def send_request_zmq(self, data):
        """
        To send request string via ZMQ and returns the response string.

        Parameters :
            data: Data request string to send via ZMQ.
        Returns :
            Response string.
        """
        logger.info("Send request to ZMQ URL: {}".format(self.zmq_url))
        replymessage = None
        try:
...
# -------------------------------------------------------------------------
    ...
    def connect(self):
        ...
        try:
            ...
            logger.info("Established connection with ZMQ URL: {}"
                        .format(self.zmq_url))
        except Exception as ex:
