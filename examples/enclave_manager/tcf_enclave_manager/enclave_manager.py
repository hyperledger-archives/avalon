        ...
        for wo in processing_list:
            ...
            if wo_json_resp is not None:
                ...
                if wo_receipt is None:
                    ...
                    logger.info(
                        "Receipt created for workorder %s during boot", wo_receipt)
    ...
    def process_work_orders(self, kv_helper):
        ...
        try:
            ...
            if not list_of_workorders:
                logger.info(
                    "Received empty list of work orders from wo-scheduled table")
                return
        ...
        for wo_id in list_of_workorders:
            ...
            logger.info(
                "Create workorder entry %s in wo-processing table", wo_id)
            kv_helper.set("wo-processing", wo_id,
                          WorkOrderStatus.PROCESSING.name)

            logger.info(
                "Delete workorder entry %s from wo-scheduled table", wo_id)
            ...
            logger.info(
                "Mark workorder status for workorder id %s to Completed in wo-processed", wo_id)
            kv_helper.set("wo-processed", wo_id,  WorkOrderStatus.SUCCESS.name)

            logger.info(
                "Create entry in wo-responses table for workorder %s", wo_id)
            kv_helper.set("wo-responses", wo_id, wo_json_resp)

            logger.info("Create workorder receipt for workorder %s", wo_id)
            receipt = create_receipt(wo_id, wo_json_resp)
            logger.info("receipt for the workorder id %s is %s",
                        wo_id, receipt)

            logger.info(
                "Create entry in wo-receipts table for workorder %s", wo_id)
            kv_helper.set("wo-receipts", wo_id, receipt)

            logger.info(
                "Delete workorder entry %s from wo-processing table", wo_id)
            kv_helper.remove("wo-processing", wo_id)
...
def create_json_worker(enclave_data, config):
    ...
    worker_info["organizationId"] = config.get("WorkerConfig")[
        "OrganizationId"]
    worker_info["applicationTypeId"] = config.get("WorkerConfig")[
        "ApplicationTypeId"]

    details_info = dict()
    details_info["workOrderSyncUri"] = config.get("WorkerConfig")[
        "WorkOrderSyncUri"]
    details_info["workOrderAsyncUri"] = config.get("WorkerConfig")[
        "WorkOrderAsyncUri"]
    details_info["workOrderPullUri"] = config.get("WorkerConfig")[
        "WorkOrderPullUri"]
    details_info["workOrderNotifyUri"] = config.get("WorkerConfig")[
        "WorkOrderNotifyUri"]
    details_info["receiptInvocationUri"] = config.get(
        "WorkerConfig")["ReceiptInvocationUri"]
    ...
    details_info["hashingAlgorithm"] = config.get("WorkerConfig")[
        "HashingAlgorithm"]
    details_info["signingAlgorithm"] = config.get("WorkerConfig")[
        "SigningAlgorithm"]
    details_info["keyEncryptionAlgorithm"] = config.get(
        "WorkerConfig")["KeyEncryptionAlgorithm"]
    details_info["dataEncryptionAlgorithm"] = config.get(
        "WorkerConfig")["DataEncryptionAlgorithm"]
    details_info["workOrderPayloadFormats"] = config.get(
        "WorkerConfig")["workOrderPayloadFormats"]
