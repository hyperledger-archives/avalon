...
class App():
    ...
    def createJsonFile(self):
        ...
        global inline_inData

        inline_inData = False ##TO ADD OPTION OF INLINING OR NOT THIS SHOULD BE CHANGED
        ...
        if not inline_inData:
            inData_json_aux = inData_json
            inData_json_aux["index"] = index
            inData_json_aux["data"] = preprocess
            index+=1
            inData_str = inData_str + json.dumps(inData_json_aux) + ",\n"

            inData_json_aux = inData_json
            inData_json_aux["index"] = index
            inData_json_aux["data"] = country + "," + str(numberUsers) + "," + str(numberTransactions)
            index+=1
            inData_str = inData_str + json.dumps(inData_json_aux) + ",\n"
            ...
            for i in range(0,numberUsers):
                inData_json_aux = inData_json
                inData_json_aux["index"] = index
                inData_json_aux["data"] = users['userId'][i] + "," + users['name'][i]
                index+=1
                inData_str = inData_str + json.dumps(inData_json_aux) + ",\n"
            ...
            for j in range(0,numberTransactions):
                inData_json_aux = inData_json
                inData_json_aux["index"] = index
                inData_json_aux["data"] = transactions['from'][j] + "," + transactions['to'][j] + "," + str(transactions['amount'][j]) + "," + transactions['currency'][j] + "," + transactions['date'][j]
                index+=1
                inData_str = inData_str + json.dumps(inData_json_aux) + ",\n"

            inData_str = inData_str[:-2] + "\n]"
        else:
            inData_json_aux = inData_json
            inData_json_aux["index"] = index
            index+=1
    ...
    def encryptJsonInData(self):
        ...
        enc_session_json = '{"key": ' + str(list(session_key)) + ',"iv": ' + str(list(session_iv)) + '}'
