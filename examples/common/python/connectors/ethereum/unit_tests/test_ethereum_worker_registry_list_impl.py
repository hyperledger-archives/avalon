...
sc_addr: %s\n application_ids: %s', hex_to_utf(self.__org_id),
            self.__uri, hex_to_utf(self.__sc_addr), pretty_ids(self.__app_type_ids))
...
sc_addr: %s\n application_ids: %s', hex_to_utf(self.__org_id),
            self.__new_uri, hex_to_utf(self.__sc_addr), pretty_ids(self.__new_app_id))
    ...
    def test_registry_set_status(self):
        ...
        logging.info('Calling registry_set_status contract..\n org_id: %s\n status: %d',
            hex_to_utf(self.__org_id), self.__new_status.value)
    ...
    def test_registry_lookup(self):
        logging.info('Calling registry_lookup..\n application_id: %s', 
            hex_to_utf(self.__app_type_ids[0]))
    ...
    def test_registry_retrieve(self):
        ...
        self.assertEqual(hex_to_utf(result[1]), 
            hex_to_utf(self.__sc_addr), 
    ...
    def test_registry_lookup_next(self):
        ...
        logging.info('Calling registry_lookup_next..\n application_id: %s\n lookup_tag: %s',
            hex_to_utf(self.__app_type_ids[0]), lookup_tag)
...
def main():
    ...
    test = TestEthereumWorkerRegistryListImpl(tcf_home + "/examples/common/python/connectors/" + "tcf_connector.toml")
