...
from connectors.interfaces.work_order_encryption_key_interface \
    import WorkOrderEncryptionKeyInterface
...
class WorkOrderEncryptionKeyJrpcImpl(WorkOrderEncryptionKeyInterface):
    ...
    def encryption_key_get(self, worker_id, last_used_key_nonce, tag,
        ...
        if not is_valid_hex_str(worker_id):
            ...
            return create_jrpc_response(id, JsonRpcErrorCode.INVALID_PARAMETER,
                                        "Invalid Worker id")
        if last_used_key_nonce is not None and \
                not is_valid_hex_str(last_used_key_nonce):
        ...
        if signature_nonce is not None and \
                not is_valid_hex_str(signature_nonce):
    ...
    def encryption_key_set(self, worker_id, encryption_key,
        ...
        if encryption_key_nonce is not None and \
                not is_valid_hex_str(encryption_key_nonce):
