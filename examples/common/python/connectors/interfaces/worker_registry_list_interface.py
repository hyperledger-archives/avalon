...
class WorkerRegistryListInterface(ABC):

    def __init__(self):
        super().__init__()

    @abstractmethod
    def registry_add(self, organization_id, uri, sc_addr, application_type_ids):
        """
    ...
    @abstractmethod
    def registry_update(self, organization_id, uri, sc_addr, application_type_ids):
        """
        Update a registry
        Inputs
        1. organization_id bytes[] identifies organization that hosts the registry,
    ...
    def registry_set_status(self, organization_id, status):
        ...
        Inputs
        1. organization_id bytes[] identifies organization that hosts the registry
        2. status defines registry status to set. The currently defined values are:
    ...
    def registry_retrieve(self, organization_id):
        ...
        1. uri is string defines a URI for this registry that supports Off-Chain Worker
        Registry JSON RPC API.
    ...
    def registry_lookup(self, application_type_id):
        ...
        1. totalCount is the total number of entries matching a specified lookup criteria.
        If this number is larger than the size of the ids array, the caller should use
        the lookupTag to call workerLookUpNext to retrieve the rest of the ids.
        2. lookupTag is an optional parameter. If it is returned, it means that there are more
        matching registry ids that can be retrieved by calling the function registry_lookup_next
        with this tag as an input parameter.
        3. ids is an array of the registry organization ids that match the input parameters
        """
        pass
    ...
    def registry_lookup_next(self, application_type_id, lookup_tag):
        ...
        This function is called to retrieve additional results of the Registry lookup initiated by
        the registryLookUp call.
        Inputs
        1. application_type_id is an application type that has to be supported by the workers retrieved.
        2. lookup_tag is returned by a previous call to either this function or to registryLookUp.

        Outputs
        1. total_count is a total number of entries matching the lookup criteria. If this number is
        larger than the number of ids returned so far, the caller should use lookup_tag to call
        registry_lookup_next to retrieve the rest of the ids.
        2. new_lookup_tag is an optional parameter. If it is returned, it means that there are more
        matching registry ids that can be retrieved by calling this function again with this tag
