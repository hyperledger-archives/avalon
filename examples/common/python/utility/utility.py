...
def generate_encrypted_session_key(session_iv, worker_encryption_key):
    ...
    Parameters:
    - worker_encryption_key is a one-time encryption key generated by the participant
      submitting the work order.
    - session_iv is an initialization vector if required by the data encryption algorithm
...
def decrypt_data(encryption_key, iv, data):
    ...
    decrypt_result = crypto.SKENC_DecryptMessage(encryption_key,
        crypto.hex_to_byte_array(iv), data_byte)
...
def decrypted_response(input_json_str, encrypted_session_key):
    ...
    for item in data_objects:
        ...
        if not do_decrypt:
            ...
            input_json_params['outData'][i]['data'] = decrypt_data(
                encryption_key_byte, item['iv'], item['data'])
        i = i + 1
