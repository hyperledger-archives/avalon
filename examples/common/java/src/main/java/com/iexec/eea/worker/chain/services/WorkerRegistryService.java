...
import com.iexec.eea.worker.contract.generated.WorkerRegistry;
...
public class WorkerRegistryService {
    ...
    @Autowired
    public WorkerRegistryService(CredentialsService credentialsService, Web3jService web3JService,
    ...
    public boolean workerRegister(String workerId,
        ...
        RemoteCall<TransactionReceipt> registerCall = getWorkerRegistryContract().workerRegister(
                workerIdBytes,
        ...
        try {
            ...
            log.error("Failed to register [workerId:{}, exception:{}]", workerId, e.getMessage());
        }
    ...
    public boolean workerUpdate(String workerId, String details) {
        ...
        RemoteCall<TransactionReceipt> updateCall = getWorkerRegistryContract().workerUpdate(
                workerIdBytes,
                details);

        try {
            updateReceipt = updateCall.send();
            log.info("Updated [workerId:{}, details:{}, gasUsed:{}]", workerId, details, updateReceipt.getGasUsed());
            return true;
        } catch (Exception e) {
            log.error("Failed to update[workerId:{}, exception:{}]", workerId, e.getMessage());
        }
    ...
    public String[] workerLookUp(int workerType, String organizationId, String appTypeId) {
        ...
        RemoteCall<Tuple3<BigInteger, String, List<byte[]>>> lookupCall = getWorkerRegistryContract().workerLookUp(
    ...
    public String[] workerLookUpNext(int workerType, String organizationId, String appTypeId, String lookupTag) {
        byte[] orgIdBytes = stringToBytes32(organizationId);
        byte[] appTypeIdBytes = stringToBytes32(appTypeId);

        Tuple3<BigInteger, String, List<byte[]>> lookupResult;
        RemoteCall<Tuple3<BigInteger, String, List<byte[]>>> lookupCall = getWorkerRegistryContract().workerLookUpNext(
                BigInteger.valueOf(workerType),
                orgIdBytes,
                appTypeIdBytes,
                lookupTag);
    ...
    public Optional<ChainWorker> workerRetrieve(String workerId) {
        ...
        RemoteCall<Tuple5<BigInteger, BigInteger, byte[], List<byte[]>, String>> retrieveCall =
                getWorkerRegistryContract().workerRetrieve(workerIdBytes);

        try {
            chainWorker = retrieveCall.send();
        } catch (Exception e) {
            log.error("Failed to retrieve [workerId:{}, exception:{}]", workerId, e.getMessage());
