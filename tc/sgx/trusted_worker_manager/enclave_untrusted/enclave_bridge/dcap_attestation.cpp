...
namespace tcf {
	...
	// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
        void DcapAttestation::CreateQuoteFromReport(const sgx_report_t* inEnclaveReport,
        ...
        sgx_status_t DcapAttestation::VerifyEnclaveInfoSingleton(const std::string& enclaveInfo,
                    const std::string& mr_enclave,
                    const sgx_enclave_id_t& enclave_id) {

            sgx_status_t result = SGX_SUCCESS;
            tcf_err_t presult = this->VerifyEnclaveInfo(enclaveInfo, enclave_id);
            tcf::error::ThrowSgxError(presult,
            ...
            sgx_status_t sresult = tcf::sgx_util::CallSgx(
                [ enclave_id,
                  ...
                  enclaveInfo,
                  mr_enclave ] () {
                      return ecall_VerifyEnclaveInfoDcap(
                             enclave_id,
                             &result,
                             enclaveInfo.c_str(),
                             mr_enclave.c_str());
        ...
        tcf_err_t DcapAttestation::VerifyEnclaveInfoKME(const std::string& enclaveInfo,
                    const std::string& mr_enclave,
                    const std::string& ext_data,
                    const sgx_enclave_id_t& enclave_id) {
            tcf_err_t presult = TCF_SUCCESS;

            presult = this->VerifyEnclaveInfo(enclaveInfo, enclave_id);
            tcf::error::ThrowIf<tcf::error::ValueError>(presult != TCF_SUCCESS,
                "Failed to verify DCAP enclave info");
        ...
        // XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
#elif BUILD_WPE
        tcf_err_t DcapAttestation::VerifyEnclaveInfoWPE(const std::string& enclaveInfo,
            const std::string& mr_enclave,
            const std::string& ext_data,
            const sgx_enclave_id_t& enclave_id) {
            tcf_err_t presult = TCF_SUCCESS;
