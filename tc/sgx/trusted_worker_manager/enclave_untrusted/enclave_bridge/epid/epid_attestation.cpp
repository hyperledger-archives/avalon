...
// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
tcf_err_t EpidAttestation::GetEnclaveCharacteristics(
...
void EpidAttestation::CreateQuoteFromReport(const sgx_report_t* inEnclaveReport,
    ...
    tcf::error::ThrowIfNull(inEnclaveReport,
...
void EpidAttestation::SetSpid(const HexEncodedString& inSpid) {
    tcf::error::ThrowIf<tcf::error::ValueError>(inSpid.length() != 32,
        "Invalid SPID length");
    tcf::HexStringToBinary(this->spid.id, sizeof(this->spid.id), inSpid);
}  // EpidAttestation::SetSpid

// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
void EpidAttestation::SetSignatureRevocationList (
    ...
    uint32_t size;
    tcf::error::ThrowSgxError(sgx_calc_quote_size(
        pRevocationList, revocationListSize, &size));
    this->quoteSize = size;
...
tcf_err_t EpidAttestation::VerifyEnclaveInfoSingleton(
    ...
    tcf::error::ThrowSgxError(sresult,
        "Intel SGX enclave call failed (ecall_VerifyEnclaveInfoSingleton)");
    return presult;
}

// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
#elif BUILD_KME
tcf_err_t EpidAttestation::VerifyEnclaveInfoKME(
    const std::string& enclaveInfo,
    const std::string& mr_enclave, const std::string& ext_data,
    const sgx_enclave_id_t& enclave_id) {
    ...
    tcf::error::ThrowSgxError(sresult,
        "Intel SGX enclave call failed (ecall_VerifyEnclaveInfoKME)");
    return presult;
}

// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
#elif BUILD_WPE
tcf_err_t EpidAttestation::VerifyEnclaveInfoWPE(
    const std::string& enclaveInfo,
    const std::string& mr_enclave,
