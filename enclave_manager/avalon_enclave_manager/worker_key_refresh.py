...
# -----------------------------------------------------------------
    ...
    def _initiate_key_refresh(self):
        ...
        try:
            ...
            self._enclave_info.encryption_key = \
                enclave_signup_data['encryption_key']
            self._enclave_info.encryption_key_signature = \
                enclave_signup_data['encryption_key_signature']
            self._enclave_info.sealed_enclave_data = \
                enclave_signup_data['sealed_enclave_data']

            # Add a new worker
            worker_info = EnclaveManager.create_json_worker(
                self._enclave_info, self._config)
            logger.info(
                "Persisting updated worker details after key refresh - %s",
                worker_info)
