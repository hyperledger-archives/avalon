...
class WPERequester():
    ...
    def get_unique_verification_key(self, nonce):
        ...
        Parameters :
            @param nonce - Random hex nonce generated by this WPE
        Returns :
    ...
    def register_wo_processor(self, attestation_report):
        ...
        Returns :
            @returns result - Result field containing the status of the
                              registration. None, in case of failure.
        """
        json_rpc_request = self._get_request_json("RegisterWorkOrderProcessor")
        json_rpc_request["params"] = {"attestation_report": attestation_report}
        response = self._post_and_get_result(json_rpc_request)
        if "result" in response:
            return response["result"]
        else:
    ...
    def preprocess_work_order(self, wo_request, encryption_key):
        ...
        if "result" in response:
            ...
            logger.error("The KME could not preprocess the work order: {}"
                         .format(response))
            return None
