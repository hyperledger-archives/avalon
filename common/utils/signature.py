...
#---------------------------------------------------------------------------------------------
    def __calculate_hash_on_concatenated_string(self, input_json_params, nonce_hash):
        ...
        workorder_id = (input_json_params['workOrderId']).encode('ASCII')
        worker_id = (input_json_params['workerId']).encode('ASCII')
        workload_id = "".encode('ASCII')
        if 'workloadId' in input_json_params :
            workload_id = (input_json_params['workloadId']).encode('ASCII')
        requester_id = (input_json_params['requesterId']).encode('ASCII')
...
#---------------------------------------------------------------------------------------------
    def __calculate_datahash(self, data_objects):
        ...
        for item in data_objects:
            datahash = "".encode('ASCII')
            if 'dataHash' in item:
                datahash = item['dataHash'].encode('ASCII')
            data = item['data'].encode('ASCII')
            e_key = item['encryptedDataEncryptionKey'].encode('ASCII')
...
#---------------------------------------------------------------------------------------------
    def generate_client_signature(self, input_json_str, worker, private_key, session_iv, encrypted_session_key):
        """
        Function to generate client signature
        Parameters:
            - input_json_str is requester Work Order Request payload in a JSON-RPC based format defined 6.1.1 Work Order Request Payload
        ...
        # Using first [NO_OF_BYTES]16 bytes of encrypted session key to encrypt data
        compressed_encrypted_session_key = encrypted_session_key[:NO_OF_BYTES]
        encrypted_session_key_str = ''.join(format(i, '02x') for i in compressed_encrypted_session_key)
        encrypted_session_key = compressed_encrypted_session_key
        self.__encrypt_workorder_indata(input_json_params, compressed_encrypted_session_key, session_iv)

        #[NO_OF_BYTES] 16 BYTES for nonce, is the recommendation by NIST to avoid collisions by the "Birthday Paradox".
        nonce =  crypto.random_bit_string(NO_OF_BYTES)

        request_nonce_hash = crypto.compute_message_hash(nonce)
        nonce_hash = (crypto.byte_array_to_base64(request_nonce_hash)).encode('ASCII')
        ...
        concat_hash = bytes(concat_string, 'ascii')
        ...
        # Temporary mechanism to share Clients public key. Not a part of Spec
...
#---------------------------------------------------------------------------------------------
    def verify_signature(self,response_str,worker):
        ...
        nonce = (input_json_params['workerNonce']).encode('ASCII')
