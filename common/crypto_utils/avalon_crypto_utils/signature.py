...
# -----------------------------------------------------------------------------
    def __encrypt_workorder_data(
            self, data_objects, session_key, session_iv,
        ...
        Parameters:
            ...
            - data_objects is inData/outData elements within the
        ...
        logger.debug("Workorder data after encryption: %s", data_objects)
...
# -----------------------------------------------------------------------------
    def generate_client_signature(
        ...
        if "inData" in input_json_params and \
                len(input_json_params["inData"]) > 0:
            logger.debug("Encrypting Workorder inData")
            in_data_objects = input_json_params['inData']
            self.__encrypt_workorder_data(
                in_data_objects, session_key, session_iv,
                worker.encryption_key, data_key, data_iv)

        if "outData" in input_json_params and \
                len(input_json_params["outData"]) > 0:
            logger.debug("Encrypting Workorder outData")
            out_data_objects = input_json_params['outData']
            self.__encrypt_workorder_data(
                out_data_objects, session_key, session_iv,
                worker.encryption_key, data_key, data_iv)
